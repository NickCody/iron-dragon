...
#define CGICDEBUGSTART \
	{ \
		FILE *dout; \
		dout = fopen("C:\\Program Files\\Apache\\htdocs\\styles\\debug.txt", "a"); \
...
static cgiParseResultType cgiParsePostFormInput() {
	...
	if (fread(input, 1, cgiContentLength, cgiIn) != (size_t)cgiContentLength) {
		return cgiParseIO;
	}	
...
static int cgiWriteString(FILE *out, char *s) {
	int len = strlen(s);
	cgiWriteInt(out, len);
	if (fwrite(s, 1, len, out) != (size_t)len) {
		return 0;
	}
	return 1;
}
